...
public class PromocionAxB extends Promocion {
		...
		for(String claveValor:config) {
			...
			switch (claveYValor[0]) {
                ...
                case "incluye":
                    ...
                    setPromo(new Atraccion[nombres.length + 1]);
                ...
                case "gratis":
                    for(Atraccion i:font) {
                        if(i.getNombre().equals(claveYValor[1])) {
                            setHoras(getHoras() + i.getHoras());
                            setCupo((i.getCupo() > getCupo())? getCupo(): i.getCupo());
                            getPromo()[getPromo().length - 1] = i;
                        }
                    }
                    break;
				default:
    ...
    public static PromocionAxB crear(String construct, Atraccion[] font) {
        PromocionAxB newer = new PromocionAxB();
